...
SELECT a.admn_no,a.course_id,a.branch_id,dc.dept_id,semester,
	CASE when b.fee_date='0000-00-00'
		then false
...
WHERE a.acad_status='1' AND a.hod_status='1' ";
...
";  
    ...
    foreach($result->result() as $row)
    {
        ...
        else if($row->physical_registration_status=='1')
        {
            $donePhysicalCount+=$row->cnt;
        }
    }
    if($donePhysicalCount)
    {
        array_push($summary,array('category'=>'Completed physical registration ','count'=>$donePhysicalCount));      
    }
    return $summary;
}
...
function get_details($params) {
    $queryParams = array();
    $sql="select a.admn_no,b.`name` AS branch,c.`name` AS course,d.`name` AS department,a.semester,a.`session`,a.`session_year`,
